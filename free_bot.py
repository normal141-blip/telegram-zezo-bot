...
def analyze(symbol: str) -> str:
    ...
    df, mode = fetch_data(symbol)  # جلب البيانات (إما من 5 دقائق أو يومي)

    # التحقق من البيانات
    if df is None or df.empty:
        return "❌ لا توجد بيانات"  # إذا كانت البيانات فارغة، العودة برسالة خطأ

    # حساب المؤشرات (RSI, EMA, VWAP)
    sig = compute_signals(df)
    # الحصول على التوصية (شراء أو بيع)
    rec, why, strength = decide_recommendation(sig)
    # بناء مستويات الدخول والوقف والأهداف
    lv = build_levels(sig, rec)

    # تحديد إطار البيانات (إما 5 دقائق أو يومي)
    if mode == "5m":
        timeframe = "5 دقائق"  # إذا كانت البيانات من 5 دقائق
    elif mode == "1d":
        timeframe = "يومي"  # إذا كانت البيانات يومية
    else:
        timeframe = mode  # إذا كان هناك نوع آخر من البيانات

    # إرجاع الرسالة المجمعة (التوصية والمعلومات)
    return (
        f"📊 {symbol} ({timeframe})\n"
        f"⏱️ الإطار: {timeframe}\n"  # الإطار الزمني (5 دقائق أو يومي)
        f"🕌 الشرعية: (مسؤوليتك أنت)\n"  # المسؤولية على المستثمر
